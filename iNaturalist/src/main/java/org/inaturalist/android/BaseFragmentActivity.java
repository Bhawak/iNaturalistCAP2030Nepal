...
import org.inaturalist.android.capnepal.QuestActivity;
import org.inaturalist.android.capnepal.QuestFirebaseMessagingService;
...
public class BaseFragmentActivity extends AppCompatActivity {
    ...
    private NotificationQuestReceiver mNotificationQuestReceiver;
	...
	private void buildSideMenu() {
        ...
        findViewById(R.id.menu_quest).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (!isNetworkAvailable()) {
                    Toast.makeText(getApplicationContext(), R.string.not_connected, Toast.LENGTH_LONG).show();
                ...
                startActivityIfNew(new Intent(BaseFragmentActivity.this, QuestActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP));
            }
        });
...
//                Toast.makeText(BaseFragmentActivity.this, "New Quest", Toast.LENGTH_SHORT).show();
//            }
//            if (getIntent().getStringExtra("messageType") != null) {
//                Toast.makeText(BaseFragmentActivity.this, "New Quest", Toast.LENGTH_SHORT).show();
//                if (getIntent().getStringExtra("messageType").equals("Quest")) {
//            Toast.makeText(BaseFragmentActivity.this, "New Quest", Toast.LENGTH_SHORT).show();
            SharedPreferences prefs = getSharedPreferences("iNaturalistPreferences", MODE_PRIVATE);
            SharedPreferences.Editor editor = prefs.edit();
            editor.putBoolean("unread_quests", true);
            editor.apply();
//                }
        }
//            for (String key : getIntent().getExtras().keySet()) {
//                Object value = getIntent().getExtras().get(key);
//                Toast.makeText(BaseFragmentActivity.this, key + value.toString(), Toast.LENGTH_SHORT).show();
...
//                Toast.makeText(BaseFragmentActivity.this, msg, Toast.LENGTH_SHORT).show();
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        if (mDrawerToggle != null) {
            ...
            mNotificationQuestReceiver = new NotificationQuestReceiver();
            IntentFilter filter4 = new IntentFilter(QuestFirebaseMessagingService.QUEST_NOTIFICATION);
            Logger.tag(TAG).info("Registering QUEST_NOTIFICATION");
            safeRegisterReceiver(mNotificationQuestReceiver, filter4);
        }
    }
    ...
    protected void onPause() {
        ...
        safeUnregisterReceiver(mNotificationQuestReceiver);
    }
    ...
    private void refreshUnreadQuest() {
        // Show the unread activities badge
        SharedPreferences prefs = getSharedPreferences("iNaturalistPreferences", MODE_PRIVATE);
        boolean unreadActivities = prefs.getBoolean("unread_quests", false);
        TextView activityBadge = (TextView)findViewById(R.id.quest_badge);
        activityBadge.setVisibility( unreadActivities ? View.VISIBLE : View.GONE);
    }
    ...
    private class NotificationQuestReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.wtf("Test", "BroadCast Received");
            String messageTitle = getIntent().getStringExtra("messageTitle");
            String messageBody = getIntent().getStringExtra("messageDescription");
            SharedPreferences prefs = getSharedPreferences("iNaturalistPreferences", MODE_PRIVATE);
            SharedPreferences.Editor editor = prefs.edit();
            editor.putBoolean("unread_quests", true);
            editor.apply();
